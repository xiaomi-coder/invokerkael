...
namespace Gui
{
	...
	inline bool Keybind(const char* str_id, KeyBind_t* keybind) {
		...
		const ImVec2 label_size = ImGui::CalcTextSize(KeyNames[keybind->m_uKey]);
		...
		std::uint32_t key = keybind->m_uKey;
		...
		if (bLeftClicked && !(g.ActiveId == id) && keybind->m_uKey != 0U)
		{
		...
		if (ImGui::BeginPopup(str_id))
		{
			...
			if (ImGui::Button(X("Hold"), ImVec2(70, 15))) keybind->m_uMode = 0U;
			if (ImGui::Button(X("Toggle"), ImVec2(70, 15))) keybind->m_uMode = 1U;
			if (ImGui::Button(X("Always-on"), ImVec2(70, 15))) keybind->m_uMode = 2U;
		...
		char buf_display[64] = "[-]";
		std::string active = std::vformat(X("[{}]"), std::make_format_args(KeyNames[keybind->m_uKey]));

		if (keybind->m_uKey != 0 && g.ActiveId != id)

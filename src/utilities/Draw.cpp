...
void Draw::RenderDrawData(ImDrawList* pDrawList)
{
	...
	for (const auto& data : m_vecSafeDrawData)
	{
		...
		case EDrawType::RING:
		{
			...
			const float flStep = (float)M_PI * 2.0f / pObject.nSegments;
			std::vector<ImVec2> vecPoints{};
			for (float a = 0; a < (M_PI * 2.0f); a += flStep)
			{
				Vector vecStart(pObject.flRadius * std::cosf(a) + pObject.vecCenter.x, pObject.flRadius * std::sinf(a) + pObject.vecCenter.y, pObject.vecCenter.z);
				Vector vecEnd(pObject.flRadius * std::cosf(a + flStep) + pObject.vecCenter.x, pObject.flRadius * std::sinf(a + flStep) + pObject.vecCenter.y, pObject.vecCenter.z);

				ImVec2 vecStart2D{};
				ImVec2 vecEnd2D{};

				if (WorldToScreen(vecStart, vecStart2D) && WorldToScreen(vecEnd, vecEnd2D))
				{
					vecPoints.emplace_back(vecStart2D);
					vecPoints.emplace_back(vecEnd2D);
				}
			}

			pDrawList->AddPolyline(vecPoints.data(), vecPoints.size(), pObject.colCircle, 0, pObject.flThickness);
			break;
		}
		default:
			break;
...
{
	m_vecDrawData.emplace_back(EDrawType::LINE, std::make_any<LineObject_t>(LineObject_t{ vecStart, vecEnd, colLine.GetU32(), flThickness }));
}
...
{
	m_vecDrawData.emplace_back(EDrawType::RECT, std::make_any<RectObject_t>(RectObject_t{ vecMin, vecMax, colRect.GetU32(), uFlags, colOutline.GetU32(), flRounding, flThickness }));
}
...
{
	m_vecDrawData.emplace_back(EDrawType::RECT_MULTICOLOR, std::make_any<RectMultiColorObject_t>(RectMultiColorObject_t{ vecMin, vecMax, colUpperLeft.GetU32(), colUpperRight.GetU32(), colBottomLeft.GetU32(), colBottomRight.GetU32(), uFlags, colOutline.GetU32() }));
}
...
{
	m_vecDrawData.emplace_back(EDrawType::CIRCLE, std::make_any<CircleObject_t>(CircleObject_t{ vecCenter, flRadius, colCircle.GetU32(), nSegments, uFlags, colOutline.GetU32(), flThickness }));
}
...
{
	m_vecDrawData.emplace_back(EDrawType::RING, std::make_any<RingObject_t>(RingObject_t{ vecCenter, flRadius, colCircle.GetU32(), nSegments, uFlags, flThickness }));
}
...
{
	m_vecDrawData.emplace_back(EDrawType::TRIANGLE, std::make_any<TriangleObject_t>(TriangleObject_t{ vecFirst, vecSecond, vecThird, colTriangle.GetU32(), uFlags, colOutline.GetU32(), flThickness }));
}
...
{
	m_vecDrawData.emplace_back(EDrawType::POLYGON, std::make_any<PolygonObject_t>(PolygonObject_t{ std::move(vecPoints), colPolygon.GetU32(), uFlags, colOutline.GetU32(), bClosed, flThickness }));
}
...
void Draw::AddText(const ImFont* pFont, float flFontSize, const ImVec2& vecPosition, const std::string& szText, const Color& colText, unsigned int uFlags, const Color& colOutline)
{
	...
	m_vecDrawData.emplace_back(EDrawType::TEXT, std::make_any<TextObject_t>(TextObject_t{ pFont, flFontSize, vecPosition, szText, colText.GetU32(), uFlags, colOutline.GetU32() }));
}
...
{
	m_vecDrawData.emplace_back(EDrawType::IMAGE, std::make_any<ImageObject_t>(ImageObject_t{ pTexture, vecMin, vecMax, colImage.GetU32(), flRounding }));
}
...
{
	m_vecDrawData.emplace_back(EDrawType::ARC, std::make_any<ArcObject_t>(ArcObject_t{ vecPosition,flRadius,flMinAngle,flMaxAngle,colFill.GetU32(),flThickness }));
}
#pragma endregion
...
bool Draw::WorldToScreen(const Vector& vecOrigin, ImVec2& vecScreen)
{
	...
	vecScreen.x = (flW * 0.5f) + (vecScreen.x * flW) * 0.5f;
	vecScreen.y = (flH * 0.5f) - (vecScreen.y * flH) * 0.5f;
	return true;
}
